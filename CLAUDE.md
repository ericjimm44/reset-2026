...
- **Today opens in focus mode: only what's LEFT.** Checked items disappear from the list. Everything else (reflection, ratings, done items) is collapsed behind taps. Never show a wall of checkboxes by default.
- **"The One Rule" card is the anti-overwhelm device.** When anxiety hits, the app asks one question: *what's the next box I can check?* and names a single box. Never remove this.
...
```
src/
  App.jsx              state owner, tabs, save/load, derived stats
  lib/
  ...
  data/
    daily.js           GROUPS, REFLECT, RATINGS, SEAL_Q
    path.js            PATH (40 weeks), SCORE (monthly scorecard)
    goals.js           YEAR milestones, TRACKERS
  components/
    Today.jsx  Trends.jsx  Path.jsx  Log.jsx  Year.jsx  ui.jsx
```

All state lives in `App.jsx` and flows down. Components are presentational + local UI state only.
